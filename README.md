...
| **Test steps** | 1 **Arrange** — prepare 8 minimal JSON stubs, one per parameter.
                   2 **Act** — call `json_builder.build(params)` with all 8 parameters.
                   3 **Assert** — verify the returned dict contains all keys from all 8 stubs |
